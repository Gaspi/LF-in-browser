...
import {Lexer, Rules, compile} from "moo";

import {
  Decl, DeclConst, DeclInj, Rew,
  CmdReq, CmdEval, CmdInfer, CmdCheckType, CmdCheckConv, CmdPrint, CmdDTree, CmdTime, CmdDebugOn, CmdDebugOff,
  PreScope, PreRef,
  Joker, Typ, Knd, All, Lam, App, Var, MVar, app
} from './term.ts'

function makeLexer(tokens: Rules, ignoreTokens: string[]) : Lexer {
  const lexer = compile(tokens);
  const ignoreSet = new Set(ignoreTokens);
  const oldnext = lexer.next;
  lexer.next = function () {
    while (true) {
      let token = oldnext.call(this);
      // moo oldnext iterator returns undefined when finished
      if (token == undefined || token.type === undefined || !ignoreSet.has(token.type)) {
        return token;
      } 
    }
  };
